...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .sidebar-header .logo { width: 36px; height: 36px; background: #3498db; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 16px; color: white; }
...
<aside class="sidebar">
    ...
    <nav class="sidebar-menu">
        ...
        {{-- BIODATA --}}
        <button class="menu-item {{ request()->routeIs('guru.siswa.*') ? 'active open' : '' }}"
        ...
        <div class="submenu {{ request()->routeIs('guru.siswa.*') ? 'open' : '' }}" id="biodata">
            <a href="{{ route('guru.siswa.index') }}" class="menu-item {{ request()->routeIs('guru.siswa.*') ? 'active' : '' }}">
                <i class="fas fa-circle fa-xs"></i> Data Siswa
            </a>
        </div>

        {{-- WALI KELAS --}}
        <button class="menu-item {{ request()->routeIs('guru.walikelas.*') ? 'active open' : '' }}"
        ...
        <div class="submenu {{ request()->routeIs('guru.walikelas.*') ? 'open' : '' }}" id="walikelas">
            ...
            <a href="{{ route('guru.walikelas.catatan') }}" class="menu-item {{ request()->routeIs('guru.walikelas.catatan*') ? 'active' : '' }}">
                <i class="fas fa-circle fa-xs"></i> Input Catatan
            </a>
        </div>

        {{-- GURU MAPEL --}}
        <button class="menu-item {{ request()->routeIs('guru.mapel.*') ? 'active open' : '' }}"
                onclick="toggleMenu('mapel')">
...
<div class="main">
    <header class="topbar">
        <div class="topbar-title">Sistem Pengolahan Rapor Siswa</div>
