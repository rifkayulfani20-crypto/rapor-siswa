            ...
            <table id="adminTable">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $adm->name }}</td>
                        <td>{{ $adm->email }}</td>
                        <td>{{ $adm->jenis_kelamin === 'L' ? 'Laki-laki' : ($adm->jenis_kelamin === 'P' ? 'Perempuan' : '-') }}</td>
                        <td>{{ $adm->nip ?? '-' }}</td>
                        <td>{{ $adm->no_hp ?? '-' }}</td>
                        <td>
                            <div style="display:flex;gap:4px;">
                                ...
                                <form method="POST" action="{{ route('admin.destroy', $adm->id) }}"
                                      style="display:inline"
                                      onsubmit="return confirm('Hapus admin {{ addslashes($adm->name) }}?')">
                                    @csrf
                                    @method('DELETE')
